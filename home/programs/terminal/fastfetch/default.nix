...
in
{
  options.customPrograms.terminal.fastfetch.enable = mkEnableOption "" // {
  ...
  config = mkIf cfg.enable {
    home = {
      ...
      file =
        ...
        in
        {
          "${config.xdg.configHome}/fastfetch/minimal.jsonc".text = escape (
            import ./minimal.nix { inherit pkgs; }
          );
          "${config.xdg.configHome}/fastfetch/minimal-droid.jsonc".text = escape (
            import ./minimal-droid.nix { inherit pkgs; }
          );
